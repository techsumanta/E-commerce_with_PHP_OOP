...
if(isset($_POST['addCart'])){
    ...
    if(setcookie('user_cart',$u_cart,time() + (3600),'/','','',TRUE)){
...
if(isset($_POST['removeCartItem'])){
    ...
    if($_COOKIE['cart_count'] == '1'){
        ...
        if(setcookie('user_cart',$u_cart,time() + (300),'/','','',TRUE)){
            setcookie('cart_count',$cart_count,time() + (300),'/','','',TRUE);            
            echo $cart_count;            
        }else{
            echo 'false';
        }
...
if(isset($_POST['addWishlist'])){
    ...
    if(!in_array($p_id,$user_wishlist,TRUE)){
    ...
    if(setcookie('user_wishlist',$u_wishlist,time() + (3600),'/','','',TRUE)){
        setcookie('wishlist_count',$wishlist_count,time() + (3600),'/','','',TRUE);
        echo $wishlist_count;        
    }else{
        echo 'false';
    }
...
if(isset($_POST['removeWishlistItem'])){
    ...
    if($_COOKIE['wishlist_count'] == '1'){                
        setcookie('wishlist_count','',time() - (300),'/','','',TRUE);
        setcookie('user_wishlist','',time() - (300),'/','','',TRUE);        
    }else{
        if(isset($_COOKIE['user_wishlist'])){
            $user_wishlist = json_decode($_COOKIE['user_wishlist']);
        ...
        $wishlist_count = count($user_wishlist);
        $u_wishlist = json_encode($user_wishlist);
        // echo json_encode(array('count' => $wishlist_count));

        if(setcookie('user_wishlist',$u_wishlist,time() + (300),'/','','',TRUE)){
            setcookie('wishlist_count',$wishlist_count,time() + (300),'/','','',TRUE);            
            echo $wishlist_count;
        }else{
            echo 'false';
        }
...
if(isset($_POST['proceedCart'])){
    ...
    if(isset($_COOKIE['user_wishlist'])){
        ...
        if(isset($_COOKIE['user_cart'])){
            ...
            $merge_array = array_merge($user_cart,$user_wishlist);
            $user_cart = array_unique($merge_array);
