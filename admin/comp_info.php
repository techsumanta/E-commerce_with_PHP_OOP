...
<div class="admin-content-container">
    ...
    <?php
    $db = new Database();
    $db->select('site_info','*',null,null,null,null);
    $result = $db->getResult();
    if ($result > 0) {
        foreach($result as $row) { ?>
            <form id="update_site_info" class="add-post-form row" method="post" enctype="multipart/form-data">
                <div class="col-md-6">
                    <div class="form-group">
                        <label for="">Site Name</label>
                        <input type="text" class="form-control site_name" name="site_name"
                               value="<?php echo $row['site_name']; ?>" placeholder="Site Name"/>
                        <input type="hidden" name="s_id" value="<?php echo $row['site_id']; ?>" />
                    </div>
                    <div class="form-group">
